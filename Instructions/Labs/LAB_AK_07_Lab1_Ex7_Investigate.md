...
10. 右下の「**すべての詳細を表示**」 ボタンを選択します。

11. ページの左側で、状態を「**アクティブ**」に変更し、「**適用**」を選択します。

12. 下にスクロールして、「タグ」エリアで「**+**」を選択し、 **RegKey** というタグを追加して「**OK**」を選択します。
...
24.	**Win2** ホストグラフィックを選択します (Windows デバイスの名前は、ラボのホスターがどのようにデプロイしたかによって異なる場合があります)。
...
29.	Win2 ホストにマウスをホバーしたときに表示されるメニューから「**Related Alert**」を選択します。

30. 関連するアラートをクリックして調べます。
