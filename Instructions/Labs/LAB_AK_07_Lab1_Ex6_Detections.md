...
14. **Parsers**フォルダーを選択し、次に **Sysmon** フォルダーを選択します。  以下のものが見えるはずです。<br><br>  Parsers/Sysmon/Sysmon-v12.0.txt

15. Sysmon-v12.0.txt ファイルを選択し内容を確認します。
...
16. 上記のステートメントを再利用します。Sentinel で **ログ** を開き、以下のような KQL ステートメントを作成します。これはすべてのレジストリキーセット値の行を表示します。次の KQL クエリを実行します。
...
17.  このステートメントを再利用できるようにするために、 「ログ」ウィンドウで、「**保存**」、「**関数として保存**」の順に選択します。次のように入力して関数を保存します。

関数名: Event_Reg_SetValue
従来のカテゴリ: Sysmon


18. 「＋」をクリックして新しい 「ログ クエリ」 タブを開きます。そして、以下の KQL ステートメントを実行します:
...
```
次に、c:\temp ディレクトリの結果のみを返す必要があります。
...
- PostMessageTeams-OnAlert を選択し、「**次: レビュー**」 をクリックします。
