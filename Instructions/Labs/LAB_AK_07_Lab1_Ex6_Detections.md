...
このタスクでは、セキュリティイベントコネクタと Sysmon がインストールされているホストで**攻撃 1** の検出を作成します。
...
2. **サインイン**ダイアログボックスで、ラボのホスティングプロバイダーから提供された管理者用の**テナント電子メール**アカウントをコピーして貼り付け、**次へ**を選択します。

3. **パスワードの入力**ダイアログ ボックスで、ラボ ホスティング プロバイダーから提供された管理者用の**テナントパスワード** をコピーして貼り付け、**サインイン**を選択します。

5. Azure ポータルの検索バーに 「*Sentinel*」 と入力し、「**Azure Sentinel**」 を選択します。

6. 先ほど作成した Azure Sentinel ワークスペースを選択します。

7. 一般セクションから **ログ** を選択します。
...
10. 結果は、3つの異なるテーブルについて示しています。   
    ...
    Device\* テーブルは、Defender for Endpoint （データコネクタ - Microsoft 365 Defender） からのものです。  Event テーブルは、データコネクタのセキュリティイベントからのものです。 
...
14. **Parsers**フォルダーを選択し、次に **Sysmon** フォルダーを選択します。  以下のものが見えるはずです。   Sysmon-v12.0.txt
...
17.  この KQL ステートメントは、他の検出ルールの KQL ステートメントでも再利用できそうです。  
    
そこで、ログウィンドウで、**保存**、**関数として保存**の順に選択します。<br>
次のように入力して保存してください。

関数名: Event_Reg_SetValue<br>
カテゴリ: Sysmon
...
```
データの収集状況によっては、膨大な行が返される可能性があります。  これは予測されていることです。  次のタスクは、特定のシナリオにフィルターをかけることです。
...
```
これにより、特定のレコードが返され、データを確認して、行を識別するために現在のクエリの何が変更できるかを確認できます

20. 脅威アクターが reg.exe を使用してレジストリキーを追加していることがわかります。  実行ディレクトリは c:\temp です。startup.bat は別の名前にすることができるのでクエリーで使用するのには向きません。次のスクリプトを実行します。

```KQL
Event_Reg_SetValue 
| where Image contains "reg.exe"
```
いい感じです。  次に、c:\temp ディレクトリの結果のみを返すようにします。
...
22. アラートについてできるだけ多くのコンテキストを提供することにより、セキュリティ運用アナリストを支援することが重要です。これには、調査グラフで使用するエンティティ（ComputerやUserなど）の投影が含まれます。  次のクエリを実行します。
...
23. 適切な検出ルールができたので、クエリのあるログウィンドウで、コマンドバーの**新しいアラートルール**を選択し、**AzureSentinel アラートを生成する**を選択します。
...
**次へ:ルール ロジックを設定** をクリックします。

25. **ルールのロジックを設定** タブで、**ルールクエリ** と **エンティティ マッピング**が既に入力されていることを確認します。
...
27. 残りのオプションは既定値のままにします。  **次へ:インシデント設定** をクリックします。
...
### タスク 2: Defender for Endpoint による 攻撃1 の検出
...
4. 結果は、3つの異なるテーブルについて示しています。
    ...
    Device\* テーブルは、Defender for Endpoint （データコネクタ-Microsoft 365 Defender） からのものです。  Event は、データコネクタの**セキュリティ イベント** からのものです。 
...
8. アラートについてできるだけ多くのコンテキストを提供することにより、セキュリティオペレーションセンターアナリストを支援することが重要です。これには、調査グラフで使用するエンティティの投影が含まれます。
...
```
9.  適切な検出ルールができたので、クエリのあるログウィンドウで、コマンドバーの**新しいアラートルール**を選択します。  次に、**Azure Sentinel アラートの生成**を選択します。
...
11. **次へ:ルール ロジックを設定** をクリックします。

12. ルールロジックの設定タブで、**ルールのクエリ** と **エンティティ マッピング** が既に入力されている必要があります。
...
14. 残りのオプションは既定値のままにします。  **次へ:インシデントの設定**  をクリックします。
...
17. 確認と作成 タブで、**作成** を選択します。

### タスク 3: セキュリティ イベント による 攻撃2 の検出

このタスクでは、**セキュリティ イベント** コネクタと Sysmon がインストールされているホストで 攻撃2 の検出を作成します。
...
5. 最初のデータソースは SecurityEvent です。特権グループへのメンバーの追加を識別するために Windows が使用するイベント ID を調査する必要があります。次の EventID と Event は、私たちが探しているものです。    
...
6. 行を展開して、レコードに関連するすべての列を表示します。  残念ながら探しているユーザー名(**theusernametoadd**)は表示されません。  問題は、ユーザー名のかわりに、セキュリティ識別子 （SID） が保存されていることがわかります。  

```
S-1-5-21-68110331-3767655699-2103652774-1003
```

次のKQLは、SIDを照合して、Administrators グループに追加された TargetUserName にデータを入力しようとします。
...
```
これは良い検出ルールのように見えます。    
UserName1 に解決されたユーザー名が出力されていることがわかります。


**注:** ラボで使用されるデータセットが小さいため、この KQL は期待される結果を返さない場合があります。

7. アラートについてできるだけ多くのコンテキストを提供することにより、セキュリティ運用アナリストを支援することが重要です。これには、調査グラフで使用するエンティティの投影が含まれます。  次のスクリプトを実行します。
...
この結果、AccountCustomEntity に Administratos に追加されたアカウントが格納されます。

8. 適切な検出ルールができたので、クエリのあるログウィンドウで、コマンドバーの**新しいアラートルール**を選択し、**Azure Sentinel アラートを作成する**を選択します。
...
- 方針: Privilege Escalation（特権エスカレーション）
- 重大度: 高

**次へ:ルール ロジックを設定** をクリックします。

10. ルールのロジックの設定タブで、**ルールのクエリ** と **エンティティ マッピング** が既に入力されていることを確認します。

11. クエリのスケジュール設定で、次のように設定します。
...
12. 残りのオプションは既定値のままにします。  **次へ:インシデントの設定** をクリックします。
...
- **PostMessageTeamsｰOnAlert** を選択します。
