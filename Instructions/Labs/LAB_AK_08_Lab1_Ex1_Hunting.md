...
9. このステートメントの目的は、C2(command and control) が一貫してビーコンを出しているかどうかを確認するための視覚化を提供することです。  3m の設定を 30 秒(30s)に調整してください。  count_ > 5 の設定を他のスレッショルドカウント(count_ > 1 など）に変更して、影響を確認します。
...
11. Azure Sentinel ポータルの 脅威管理領域で「**ハンティング**」ページを選択します。
...
15. エンティティマッピングに、エンティティの型として **Host** を選択し、ID として **Hostname** を、値として**Device Name** を選択します。

16. 戦略と戦法で、**Command and Control** を選択して 「**作成**」を選択します。
...
25. Azure Sentinel portal のハンティングページに戻ります (ヒント: 左にスクロールします)。
