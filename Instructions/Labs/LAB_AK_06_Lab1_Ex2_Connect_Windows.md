...
7. ドロップダウンで *Microsoft Windows10* を選択します。  次に、一覧から「**Windows 10　Enterprise version 20H2**」を選択し **作成** を選択します。
...
1. 前のタスクで作成したリストから **AZWIN01** 仮想マシンを選択し、「**接続**」を選択します。*接続しています...* メッセージが完了するまで待ちます。

1. 一つ上の階層に戻り、「**仮想マシン**」ページに表示されている AZWIN01 が LogAnalytics と接続されていることを確認します。

### タスク 3: 非 Azure Windows マシンの接続
...
6. 「**データ コネクタ**」 を選択し、リストから 「**レガシ エージェントを使用したセキュリティ イベント**」 コネクタを選択します。
...
15. エージェントのセットアップ オプションプロンプトで、「**Connect the agent to Azure Log Analytics(OMS)**」 オプションを選択し、「**次へ**」を選択します。
...
引き続き **WIN2** 仮想マシンに接続する必要があります。  次の手順では、既定構成でSysmonをインストールします。.  実稼働マシンで使用するSysmonのコミュニティベースの構成を調査する必要があります
...
4. タスクバーの検索ボックスに*cmd*を入力します。  検索結果には、コマンド プロンプト アプリが表示されます。  コマンド プロンプト アプリを右クリックし、「**管理者として実行**」 を選択します。  表示されるユーザー アカウント制御のプロンプトを確認します。
...
8. 右側の「**Raw**」ボタンをクリックし、そのファイルの内容を Github から作成した sysmon.xml メモ帳ファイルにコピーしてファイルを**保存**します。
...
### タスク 5: Win1 仮想マシンを Microsoft Defender for Endpoint ディバイスをオンボードする。

このタスクでは、Win1 仮想マシンを Microsoft Defender for Endpoint にオンボードします。

**注:** **このコースの最初のモジュールでラボを完了し、Win1 仮想マシンをオンボード済みの場合は、このタスクをスキップしてください。**
